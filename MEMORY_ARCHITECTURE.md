...
2. **Deterministic first-party / third-party write gate in `sync_turn`** — before `add()`, drop third-party turns (queries/"what do we have"/run-id lookups/colleague-named talk) and lab-fact-looking content; keep only personal prefs + interest pointers. Kills the M55 query-log class at the source. Reuse the existing run-id / find_file intent regexes. *(= Option 2, deterministic tier, now with the first-party definition from Part 2.)*
   - **Resolved 2026-07-17: LLM-extract → deterministic fact-level POST-filter (a hook), not prompt-only, and not a pre-gate.** Refined from the user's insight that classifying a raw multi-intent *turn* deterministically is hard, but classifying an *extracted, atomic fact* is easy. Flow: let Mem0's built-in extraction (the prompt) surface candidate facts — it's good at the messy NLU of "is there a durable fact here?"; then a **deterministic classifier inspects each returned fact and deletes the third-party ones** before they can be retrieved. `add(infer=True, metadata=…)` returns `{"results":[{"id","memory","event"}]}`; for each `event=="ADD"` whose `memory` text the classifier flags third-party (query-log phrasing "user asked/wants to know about…", a colleague/roster name, "the group/lab", a run/sample the user doesn't own), call `delete(id)`. Runs in the existing daemon write thread, off the reply path, before any retrieval — the transient store→delete is invisible.
     ...
     - **Classifier BUILT + OFFLINE-VERIFIED 2026-07-17 (commit 787a93f).** `hermes/plugins/qnoe_rag/memory_gate.py` — pure-stdlib, standalone-testable. `classify_fact(fact) -> (keep|drop, reason)` / `should_drop(fact)`. Policy: **KEEP** personal prefs + personal details + first-party focus/interest POINTERS ("Prefers bullets", "Name is John", "Sample is Tip5Sample9", "Interested in BLG", "Is a member of the QTM subteam") + generic non-lab facts; **DROP** lab RECORDS (run id / `.db` / qcodes), lab QUERY-LOGS (lookup verb + lab object = the M55 class), THIRD-PARTY work (collective-as-subject or another person's possessive of a lab object). **Calibrated to real Mem0 phrasing** (subject-less predicates from the installed FACT_RETRIEVAL_PROMPT — "Looking for a restaurant", "Is a Software engineer" — so it matches verbs/objects, not a "User asked" prefix; Mem0 being user-centric means "Sample is X" is already first-party). High-precision by design (generic lookups like "restaurant" are KEPT; only lab-object lookups drop). Test corpus `redteam/grounding/test_memory_gate.py` = 29 cases (real Mem0 phrasings + records/query-logs/third-party + FP guards), all PASS. **Known gap (sentinel in the test):** bare-name third-party ("Peio is measuring X") currently KEEPS — Mem0's user-centric extraction rarely emits it; left to the deferred LLM party-signal.
     - **REMAINING — wiring (deferred to gateway-up, task #15).** In `sync_turn`, after `add(infer=True, metadata=…)` returns `{"results":[{id,memory,event}]}`, loop the results: for each `event=="ADD"` where `should_drop(memory)`, call `delete(id)` and log the reason; route `UPDATE`/`DELETE` events to the nightly audit instead of auto-deleting (a merge would take a good fact with it). Behind an env toggle (default OFF until live-verified). Deploy `memory_gate.py` + `__init__.py` together; live-verify with a real turn (a "where is run X" ask leaves NO fact; a "my sample is Y" leaves one).
     - **Can the LLM label first/third-party in the extraction? (2026-07-17)** Partially, as a *signal* — not a clean field. Mem0 2.0.11 extraction returns a **flat list of fact strings** (`{"facts":[...]}`, parser takes each as a string), so there is **no slot for a structured per-fact `party` field** — emitting objects breaks the parser. Routes: (a) embed a `[third]`/`[first]` prefix in the fact string (hacky — pollutes stored text, needs stripping, gpt-oss must emit it reliably); (b) a **separate lightweight classification pass** over the returned facts *with the original message as context* (clean, doesn't touch extraction, +1 off-path LLM call). If used, it MUST be combined via "**drop if EITHER the label OR the deterministic rules say third-party; missing/garbled label = no signal**" — so it can only *add* drops, never override a deterministic drop or be the sole gate (prompt-only = the M55 trap; gpt-oss mislabels + truncates JSON, hence the 512→1536 `max_tokens` bump). **Adding output structure also risks degrading extraction itself** (more truncation) — another reason to prefer route (b). **Net:** Mem0 already user-frames facts ("User asked about X"), so the safety-critical junk (query-logs) is deterministically catchable by phrasing; the LLM label mainly buys *precision* (genuine interest vs passing mention), not safety. **Recommendation:** ship the deterministic filter first (it's the guarantee), use the provenance `src_msg` to MEASURE what slips, and add the LLM party-signal (route b) only if the audit shows a real precision gap. Don't gold-plate upfront.
