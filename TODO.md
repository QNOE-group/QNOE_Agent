...
- [ ] **User allowlist — restrict who can talk to the agent.** Today `GATEWAY_ALLOW_ALL_USERS=true` in `scripts/start_hermes.sh` — ANY Teams user who can DM the bot gets full agent access (tools, RAG, terminal). Task: (1) collect the permitted users' Azure AD ids (same ids as `hermes/config/user_profiles.yaml`; unknown users' ids appear in the gateway log on first message); (2) flip `GATEWAY_ALLOW_ALL_USERS=false` and configure the gateway allowlist (check the `teams_polling` plugin / gateway docs for the expected env or config key — the flag's existence implies native support); (3) verify: permitted user gets replies, non-permitted user gets a polite refusal (and appears in the log), nothing leaks; (4) document the onboarding step "add new member to allowlist + profile map" in [[memory/deploy-patterns]]. Security-relevant before wider rollout — the agent reads the entire lab data server.
