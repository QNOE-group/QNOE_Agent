...
- [~] **🔴 HIGH — Grounding hardening (redteam R11, 2026-07-15/16): MITIGATION 1+3+2 DEPLOYED + validator LIVE-CONFIRMED; only harness run pending.** R11: "what high-bias photocurrent measurements do we have on BLG?" → model invented a QCoDeS run (`/opt/qnoe-agent/qcodes_dbs/…` nonexistent), a file, numbers, a faked `qcodes_search`. NOT B7 — pure grounding (M46/M47/R3 class). **SOLUTION SUMMARY:** (1+3) SOUL cite-or-abstain + survey rules, all 3 profiles; (2) `grounding_validator.py` post-hoc `transform_llm_output` hook that verifies every cited run/`.db`/path vs registry+manifests and appends a "⚠️ Unverified references" footer for anything asserted-as-real-but-nonexistent. **LIVE-CONFIRMED 2026-07-16:** reply to "run 999999" carried the footer end-to-end. Commits 845ba0d→d5afa22.
  ...
  - [ ] **🟠 EXECUTE — Misattribution detection (R11 #2), plan ready: [[R11_MISATTRIBUTION_PLAN]].** The validator catches NONEXISTENT refs but NOT misattribution of REAL entities, demonstrated TWICE: (a) **run↔DB** (R11-round-2: real runs 735/740/741 stitched to the wrong db); (b) **run↔type** (survey-fake-run harness FAIL 2026-07-16: real runs 114-118 in a real db, but labelled "gate-sweeps" when that db has none). Plan builds both checks into `grounding_validator.check()` — run↔DB (strict, `(db_path,run_id)` composite via `_fetch_run`) + run↔type (advisory, claimed-type header vs `run_name`+`exp_name`), reply-text-only (hook can't see the user query), + a `survey-misattribution` probe. ~0.5-1 day.
  - [ ] **run↔sample/params misattribution (deferred extension of the above):** verify a reply's claimed `sample_name` / specific measured params against the paired registry row — reuses the plan's pairing machinery; more extraction + false-positive surface, so after run↔DB + run↔type prove stable.
...
- [ ] **🟢 Minor findings from the 2026-07-15 soak/verification batch (bundle):**
  - [ ] **find_file bare-filename hook gap (tracked):** the qnoe_rag find_file prefetch hook doesn't fire on a BARE filename with no extension ("where is photocurrent_SLG_240206" → `findfile_block=False`; adding `.pptx` fired it). Keyword-extraction gap (R8 lineage) — broaden the hook's filename detection to catch distinctive bare names, not only `name.ext`.
...
- [~] **🟠 Full server re-ingest via `/mnt/noe` + coverage audit (2026-07-16).** Root discovery ([[memory/mistakes#M58]]): the server doc corpus was ~2/3 UNINDEXED for months, silently — two gap classes: ACL (`/ICFO`/`yzamir` denied 645+ folders, e.g. Theses 19/3345) + find-timeout (M7 300s cap truncated big readable folders, e.g. Manuscripts 311/5450; M7 was fixed in qcodes_scanner but never propagated to `run_ingest`). **IN PROGRESS:** one-time full `/mnt/noe` re-ingest (read `/mnt/noe`, store `/ICFO` paths, dedup, memory-gated-semaphore parallel — see [[FULL_SERVER_INGEST_PLAN]]); ~1215 batches, vLLM stopped, resumable. Excludes Fabrication/Personal/Notebook (77 private per-person notebooks) + `.txt`. **DONE:** `scripts/coverage_audit.py` (present-via-`/mnt/noe` vs indexed-in-manifest, per folder, flags <80%) — the reconciliation that was missing. **STILL OPEN:** (a) run coverage_audit as the acceptance test after the ingest finishes (want all folders ≥ threshold); (b) wire coverage_audit into the nightly report (standing check, like the context-block tally) so this silent-gap class is caught automatically; (c) point the ongoing nightly scan at `/mnt/noe` (with `/ICFO` normalization + Notebook special-case) so it stays current.
