...
- **`gpt-oss-120b` on disk, NOT served (2026-07-10):** `/opt/qnoe-agent/models/gpt-oss-120b` (65 GB MXFP4 MoE, `GptOssForCausalLM`). Piloted as a Hermes-3 replacement but its 60.8 GiB weights + KV pool **overcommit the 128 GB unified box** and hung it in swap-death twice (see [[memory/mistakes#M39]], [[memory/decisions#D14]]). Kept for a possible **supervised** retry only (`--gpu-memory-utilization ~0.55 --max-model-len 65536 --enforce-eager --max-num-seqs 2`, launch only with ≥110 GB free). This vLLM 0.22.1 auto-detects `gpt_oss_mxfp4`, auto-selects the **MARLIN** MoE backend, and auto-sets the `openai_gptoss` reasoning parser + harmony tool path (no `--tool-call-parser`/`--reasoning-parser` flags needed). `VLLM_MXFP4_BACKEND` env var is unknown/ignored by this build.
- **Unified-memory gotcha:** vLLM `--gpu-memory-utilization` budgets against **total** device memory (128 GB), NOT free RAM — it does not subtract Qdrant (~8 GB) + OS. On this box, always leave that headroom or it OOMs/thrashes.
