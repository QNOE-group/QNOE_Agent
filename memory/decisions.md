...
## D20 — Memory architecture: Cognee corpus knowledge-graph (L5) + Mem0 thinned to personalization (2026-07-17)

**Decision:** The memory system splits into two layers with different owners of truth: (1) a **Cognee** corpus knowledge-graph — a map of the group's *research program* (concepts, questions, techniques, setups, projects, findings) anchored to the measurement data — as the authoritative **L5** layer, absorbing L1/L2 retrieval + the deferred Kùzu KG; (2) **Mem0** thinned to **per-user personalization only** (prefs + interest pointers, no lab-fact payloads). The grounding validator stays the output oracle. The boundary between them is **first-party** (user is a principal → Mem0) vs **third-party** (observer of others'/group work → Cognee only), NOT subjective/objective.
**Why / sub-decisions:**
- **Framework = Cognee** over LightRAG / MS GraphRAG / Graphiti (sourced comparison in [[COGNEE_PLAN]] §1). Decider: **`add_data_points()`** loads the 76k-row QCoDeS registry as typed nodes *deterministically, no LLM* — which also sidesteps the ~47 tok/s extraction cliff — plus native Qdrant reuse + Kùzu default (matches L5) + typed Pydantic ontology + provenance. LightRAG = soft ontology + <50 tok/s timeout risk; Graphiti can't reuse Qdrant (Milvus-only) + Kùzu deprecated; GraphRAG batch/Parquet/impractical-local.
- **Two tiers of truth** ([[COGNEE_ONTOLOGY]]): Tier 1 factual anchor (registry, deterministic = ground truth, the *scaffold*); Tier 2 research/conceptual (`cognify`/LLM-inferred = the *heart* AND the risk — a weak model confabulates a research graph). Tier-2 nodes are provenance-tagged, never asserted as fact. The conceptual layer is LLM-bound for *every* framework, so the real lever is the extraction **model + validation**, not the framework.
- **`cognify` at `reasoning_effort:high`** (per-request; live gateway stays low), scoped to research-bearing docs **sourced FROM Qdrant** (reconstruct docs by `source`+`start_line` — no CIFS/SharePoint re-read; reuse the parse output). Cost = DGX wall-clock only, one-time + cheap incremental.
