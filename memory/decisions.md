...
**Decision: option 2 — systemd drop-in on `qnoe-hermes.service`** (`50-b7-readonly.conf`): `ReadOnlyPaths=/opt/qnoe-agent /ICFO /home/yzamir`, rw carve-outs `memory/ logs/ hermes/`, `InaccessiblePaths=secrets/` with `LoadCredential=teams.env`. Mount namespace binds ALL tools (file tools + every terminal child) — same guarantee OpenShell would give for the filesystem.
