...
# GW_IP = docker bridge gateway as seen from inside the sandbox (verified in
# migration Stage 3; llama.cpp binds 0.0.0.0:8000, Qdrant publishes 0.0.0.0:6333).
...
# /run/qnoe/teams.env; start_hermes.sh sources it via $TEAMS_ENV_FILE.
# Never pass secrets via --env (leaks through ps/gateway db).
exec openshell sandbox create \
    --name qnoe-hermes \
    --from qnoe-hermes:0.1 \
    --policy /opt/qnoe-agent/config/sandbox-policy.yaml \
    --env QDRANT_URL="http://${GW_IP}:6333" \
    --env VLLM_BASE_URL="http://${GW_IP}:8000/v1" \
